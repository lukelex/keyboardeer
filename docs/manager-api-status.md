...
| Compile behavior and live preview | `validation.preview` | Yes | Partial | The editor compiles and previews its complete saved draft after a Base-layer edit. The manager must render a complete runnable `defcfg` before model previews can succeed; add debouncing/concurrency controls, diagnostics adapter, and broader editor UI. |

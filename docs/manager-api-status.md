...
| `configuration.set_enabled` / `configuration.delete` | Implemented by `f8ea0ec`. Both require expected revision. | Enable/disable implemented; deletion is not yet exposed. | Keyboard cards expose a direct checked/unchecked state for manager-owned bindings. External configurations remain read-only. |
...
| Enable or disable managed runtime config | `configuration.set_enabled` | Yes | Partial | Keyboard cards use a fresh snapshot revision and direct checkbox state. Deletion remains unavailable. |

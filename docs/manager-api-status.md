...
| `configuration.create` / `configuration.update` | Implemented. Create accepts name/model; update requires configuration ID and expected revision. | Implemented. | A validated draft enters an explicit review step; updates read the current authoritative desired revision and submit it as `expected_revision`. |
| `configuration.set_enabled` / `configuration.delete` | Implemented by `f8ea0ec`. Both require expected revision. | Enable/disable implemented; deletion is not yet exposed. | Keyboard cards can stop or resume manager-owned bindings after explicit confirmation. External configurations remain read-only. |
...
| Apply a managed profile | `configuration.create` / `update`, then snapshot/events | Mostly | Partial | The editor requires a current valid preview, connected device, manager capability, and explicit confirmation. Create/update performs manager-owned validation and activation. A transport failure after dispatch is persisted as an unknown outcome and deliberately blocks retry because manager-side idempotency is not evidenced. |
| Enable or disable managed runtime config | `configuration.set_enabled` | Yes | Partial | Keyboard cards use a fresh snapshot revision and explicit confirmation. Deletion remains unavailable. |

...
| `configuration.content.get` | Implemented in v1.1.0; bounded external UTF-8 read requires the current content revision and returns a digest. | Implemented; external source is displayed through the revision-checked manager binding. | Read-only external source display; manager paths are not exposed. |
...
| Export generated device-specific `.kbd` | Implemented in v1.1.0 as `configuration.export` with `manager_rendered_kbd`. | **Yes** | Yes | GUI views and saves the manager-rendered device-bound artifact; manager owns `defcfg` rendering. |
